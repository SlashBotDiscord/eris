...
class Shard extends EventEmitter {
    ...
    createGuild(_guild) {
        ...
        if (
            this.client.options.getAllUsers &&
    ...
    disconnect(options = {}, error) {
        ...
        if (
            this.sessionID &&
            this.connectAttempts >= this.client.options.maxResumeAttempts
        ) {
            this.emit(
                "debug",
                `Automatically invalidating session due to excessive resume attempts | Attempt ${this.connectAttempts}`,
                this.id
            );
            this.sessionID = null;
        }
    ...
    editStatus(status, activities) {
        ...
        if (activities !== undefined) {
            if (
                activities.length > 0 &&
                !activities[0].hasOwnProperty("type")
            ) {
    ...
    getGuildMembers(guildID, timeout) {
        if (this.getAllUsersCount.hasOwnProperty(guildID)) {
            throw new Error(
                "Cannot request all members while an existing request is processing"
            );
        }
        ...
        if (this.client.options.intents) {
            if (
                !(this.client.options.intents & Constants.Intents.guildMembers)
            ) {
                throw new Error(
    ...
    identify() {
        if (this.client.options.compress && !ZlibSync) {
            ...
            this.emit(
                "error",
                new Error("pako/zlib-sync not found, cannot decompress data")
            );
            return;
        }
    ...
    onPacket(packet) {
        if (
            this.listeners("rawWS").length > 0 ||
            this.client.listeners("rawWS").length
        ) {
            /**
        ...
        if (packet.s) {
            if (
                ...
                this.emit(
                    "warn",
                    `Non-consecutive sequence (${this.seq} -> ${packet.s})`,
                    this.id
                );
            }
            this.seq = packet.s;
    ...
    requestGuildMembers(guildID, options) {
        ...
        if (
            opts.presences &&
            this.client.options.intents &&
            !(this.client.options.intents & Constants.Intents.guildPresences)
        ) {
            throw new Error(
                "Cannot request members presences without guildPresences intent"
            );
        }
        ...
        if (this.requestMembersPromise !== undefined) {
            for (const guildID in this.requestMembersPromise) {
                ...
                this.requestMembersPromise[guildID].res(
                    this.requestMembersPromise[guildID].received
                );
            }
        }
    ...
    restartGuildCreateTimeout() {
        ...
        if (!this.ready) {
            if (
                this.client.unavailableGuilds.size === 0 &&
                this.unsyncedGuilds === 0
            ) {
        ...
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
            ...
            const func = () => {
                if (
                    ...
                    const data = Erlpack
                        ? Erlpack.pack({ op: op, d: _data })
                        : JSON.stringify({ op: op, d: _data });
                    this.ws.send(data);
                    if (_data.token) {
                        delete _data.token;
                    }
    ...
    wsEvent(packet) {
        switch (
            packet.t /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
        ) {
            case "PRESENCE_UPDATE": {
                if (packet.d.user.username !== undefined) {
                    ...
                    if (
                        user &&
                        (user.username !== packet.d.user.username ||
                            user.discriminator !==
                ...
                if (!packet.d.guild_id) {
                    ...
                    const relationship = this.client.relationships.get(
                        packet.d.id
                    );
                    if (!relationship) {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        "Rogue presence update: " + JSON.stringify(packet),
                        this.id
                    );
                    break;
                }
                let member = guild.members.get(
                    (packet.d.id = packet.d.user.id)
                );
            ...
            case "VOICE_STATE_UPDATE": {
                ...
                if (
                    ...
                    if (voiceConnection) {
                        if (packet.d.channel_id === null) {
                            this.client.voiceConnections.leave(
                                packet.d.guild_id
                            );
                        } else if (
                            voiceConnection.channelID !== packet.d.channel_id
                        ) {
                            voiceConnection.switchChannel(
                                packet.d.channel_id,
                                true
                            );
                        }
                ...
                if (packet.d.guild_id === undefined) {
                    ...
                    if (packet.d.channel_id === null) {
                        ...
                        if (!flag) {
                            for (const privateChannel of this.client
                                .privateChannels) {
                                const call =
                                    privateChannel[1].call ||
                                    privateChannel[1].lastCall;
                                if (call && call.voiceStates.remove(packet.d)) {
                                    flag = true;
                                    break;
                                }
                            ...
                            if (!flag) {
                                this.emit(
                                    "debug",
                                    new Error(
                                        "VOICE_STATE_UPDATE for user leaving call not found"
                                    )
                                );
                                break;
                            }
                        ...
                        if (!channel.call && !channel.lastCall) {
                            this.emit(
                                "debug",
                                new Error(
                                    "VOICE_STATE_UPDATE for untracked call"
                                )
                            );
                            break;
                        }
                        (channel.call || channel.lastCall).voiceStates.update(
                            packet.d
                        );
                    }
                    break;
                ...
                if (!member) {
                    ...
                    const channel = guild.channels.find(
                        (channel) =>
                            ...
                            channel.voiceMembers.get(packet.d.id)
                    );
                    if (channel) {
                        channel.voiceMembers.remove(packet.d);
                        this.emit(
                            "debug",
                            "VOICE_STATE_UPDATE member null but in channel: " +
                                packet.d.id,
                            this.id
                        );
                    }
                ...
                if (oldChannelID != packet.d.channel_id) {
                    ...
                    if (oldChannelID) {
                        ...
                        if (
                            oldChannel &&
                            oldChannel.type !== ChannelTypes.GUILD_VOICE &&
                            oldChannel.type !== ChannelTypes.GUILD_STAGE_VOICE
                        ) {
                            this.emit(
                                "warn",
                                "Old channel not a recognized voice channel: " +
                                    oldChannelID,
                                this.id
                            );
                            oldChannel = null;
                    ...
                    if (
                        ...
                        (newChannel.type === ChannelTypes.GUILD_VOICE ||
                            newChannel.type === ChannelTypes.GUILD_STAGE_VOICE)
                    ) {
                        ...
                        if (oldChannel) {
                            ...
                            this.emit(
                                "voiceChannelSwitch",
                                newChannel.voiceMembers.add(member, guild),
                                newChannel,
                                oldChannel
                            );
                        } else {
            ...
            case "MESSAGE_CREATE": {
                ...
                if (channel) {
                    ...
                    this.emit(
                        "messageCreate",
                        channel.messages.add(packet.d, this.client)
                    );
                } else {
                    this.emit(
                        "messageCreate",
            ...
            case "MESSAGE_UPDATE": {
                ...
                this.emit(
                    "messageUpdate",
                    channel.messages.update(packet.d, this.client),
                    oldMessage
                );
                break;
            }
            case "MESSAGE_DELETE": {
                ...
                this.emit(
                    "messageDelete",
                    (channel && channel.messages.remove(packet.d)) || {
                        id: packet.d.id,
                        channel: channel || {
                            id: packet.d.channel_id,
            ...
            case "MESSAGE_REACTION_ADD": {
                ...
                if (channel) {
                    ...
                    if (channel.guild) {
                        if (packet.d.member) {
                            // Updates the member cache with this member for future events.
                            packet.d.member.id = packet.d.user_id;
                            member = channel.guild.members.update(
                                packet.d.member,
                                channel.guild
                            );
                        }
                ...
                if (message) {
                    const reaction = packet.d.emoji.id
            ...
            case "GUILD_MEMBER_ADD": {
                ...
                if (!guild) {
                    ...
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in GUILD_MEMBER_ADD`
                    );
                    break;
                }
                packet.d.id = packet.d.user.id;
            ...
            case "GUILD_MEMBER_UPDATE": {
                ...
                if (
                    !(
                        this.client.options.intents &
                        Constants.Intents.guildPresences
                    ) &&
                    packet.d.user.username !== undefined
                ) {
                    let user = this.client.users.get(packet.d.user.id);
                    let oldUser = null;
                    if (
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in GUILD_MEMBER_UPDATE`
                    );
                    break;
                }
                let member = guild.members.get(
            ...
            case "GUILD_UPDATE": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Guild ${packet.d.id} undefined in GUILD_UPDATE`
                    );
                    break;
                }
                const oldGuild = {
            ...
            case "GUILD_ROLE_CREATE": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in GUILD_ROLE_CREATE`
                    );
                    break;
                }
                this.emit(
                    "guildRoleCreate",
                    guild,
                    guild.roles.add(packet.d.role, guild)
                );
                break;
            }
            case "GUILD_ROLE_UPDATE": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Guild ${packet.d.guild_id} undefined in GUILD_ROLE_UPDATE`
                    );
                    break;
                }
                ...
                if (!role) {
                    this.emit(
                        "debug",
                        `Role ${packet.d.role} in guild ${packet.d.guild_id} undefined in GUILD_ROLE_UPDATE`
                    );
                    break;
                }
                const oldRole = {
                ...
                this.emit(
                    "guildRoleUpdate",
                    guild,
                    guild.roles.update(packet.d.role, guild),
                    oldRole
                );
                break;
            }
            case "GUILD_ROLE_DELETE": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in GUILD_ROLE_DELETE`
                    );
                    break;
                }
                if (!guild.roles.has(packet.d.role_id)) {
                    this.emit(
                        "debug",
                        `Missing role ${packet.d.role_id} in GUILD_ROLE_DELETE`
                    );
                    break;
                }
                this.emit(
                    "guildRoleDelete",
                    guild,
                    guild.roles.remove({ id: packet.d.role_id })
                );
                break;
            }
            case "INVITE_CREATE": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in INVITE_CREATE`
                    );
                    break;
                }
                const channel = this.client.getChannel(packet.d.channel_id);
                if (!channel) {
                    this.emit(
                        "debug",
                        `Missing channel ${packet.d.channel_id} in INVITE_CREATE`
                    );
                    break;
                }
                /**
            ...
            case "INVITE_DELETE": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in INVITE_DELETE`
                    );
                    break;
                }
                const channel = this.client.getChannel(packet.d.channel_id);
                if (!channel) {
                    this.emit(
                        "debug",
                        `Missing channel ${packet.d.channel_id} in INVITE_DELETE`
                    );
                    break;
                }
                /**
            ...
            case "CHANNEL_CREATE": {
                ...
                if (packet.d.guild_id) {
                    if (!channel.guild) {
                        channel.guild = this.client.guilds.get(
                            packet.d.guild_id
                        );
                        if (!channel.guild) {
                            this.emit(
                                "debug",
                                `Received CHANNEL_CREATE for channel in missing guild ${packet.d.guild_id}`
                            );
                            break;
                        }
                    }
                    channel.guild.channels.add(channel, this.client);
                    this.client.channelGuildMap[packet.d.id] =
                        packet.d.guild_id;
                    /**
            ...
            case "CHANNEL_UPDATE": {
                ...
                if (channel instanceof GroupChannel) {
                    ...
                    this.emit(
                        "warn",
                        `Unexpected CHANNEL_UPDATE for channel ${packet.d.id} with type ${oldType}`
                    );
                }
                ...
                if (oldType === packet.d.type) {
                    ...
                    this.emit(
                        "debug",
                        `Channel ${packet.d.id} changed from type ${oldType} to ${packet.d.type}`
                    );
                    ...
                    if (packet.d.guild_id) {
                        ...
                        if (!guild) {
                            this.emit(
                                "debug",
                                `Received CHANNEL_UPDATE for channel in missing guild ${packet.d.guild_id}`
                            );
                            break;
                        }
                        ...
                        if (channel instanceof GroupChannel) {
                            ...
                            this.client.groupChannels.add(
                                newChannel,
                                this.client
                            );
                        } else {
                            this.client.privateChannels.remove(channel);
                            this.client.privateChannels.add(
                                newChannel,
                                this.client
                            );
                        }
            ...
            case "CHANNEL_DELETE": {
                if (
                    packet.d.type === ChannelTypes.DM ||
                    packet.d.type === undefined
                ) {
                    if (this.id === 0) {
                        const channel = this.client.privateChannels.remove(
                            packet.d
                        );
                        if (channel) {
                            delete this.client.privateChannelMap[
                    ...
                    if (!guild) {
                        this.emit(
                            "debug",
                            `Missing guild ${packet.d.guild_id} in CHANNEL_DELETE`
                        );
                        break;
                    }
                    ...
                    if (this.id === 0) {
                        this.emit(
                            "channelDelete",
                            this.client.groupChannels.remove(packet.d)
                        );
                    }
                } else {
                    this.emit(
            ...
            case "CHANNEL_RECIPIENT_ADD": {
                const channel = this.client.groupChannels.get(
                    packet.d.channel_id
                );
                /**
            ...
            case "CHANNEL_RECIPIENT_REMOVE": {
                ...
                this.emit(
                    ...
                    channel,
                    channel.recipients.remove(packet.d.user)
                );
                break;
            }
            ...
            case "GUILD_MEMBERS_CHUNK": {
                ...
                if (!guild) {
                    this.emit(
                        ...
                        `Received GUILD_MEMBERS_CHUNK, but guild ${packet.d.guild_id} is ` +
                            (this.client.unavailableGuilds.has(
                                packet.d.guild_id
                            )
                                ? "unavailable"
                                : "missing"),
                ...
                if (this.requestMembersPromise.hasOwnProperty(packet.d.nonce)) {
                    this.requestMembersPromise[packet.d.nonce].members.push(
                        ...members
                    );
                }
                ...
                if (packet.d.chunk_index >= packet.d.chunk_count - 1) {
                    if (
                        this.requestMembersPromise.hasOwnProperty(
                            packet.d.nonce
                        )
                    ) {
                        clearTimeout(
                            this.requestMembersPromise[packet.d.nonce].timeout
                        );
                        this.requestMembersPromise[packet.d.nonce].res(
                            this.requestMembersPromise[packet.d.nonce].members
                        );
            ...
            case "GUILD_SYNC": {
                ...
                for (const presence of packet.d.presences) {
                    if (!guild.members.get(presence.user.id)) {
                        ...
                        this.emit(
                            "debug",
                            `Presence without member. ${presence.user.id}. In global user cache: ${userData}. ` +
                ...
                if (
                    ...
                    guild.pendingVoiceStates.length > 0
                ) {
                    for (const voiceState of guild.pendingVoiceStates) {
                        ...
                        if (channel) {
                            channel.voiceMembers.add(
                                guild.members.update(voiceState)
                            );
                            if (
                                this.client.options.seedVoiceConnections &&
                                voiceState.id === this.client.user.id &&
                                !this.client.voiceConnections.get(
                                    channel.guild ? channel.guild.id : "call"
                                )
                            ) {
                                this.client.joinVoiceChannel(channel.id);
                            }
                        } else {
                            // Phantom voice states from connected users in deleted channels (╯°□°）╯︵ ┻━┻
                            this.client.emit(
                                "debug",
                                "Phantom voice state received but channel not found | Guild: " +
                                    guild.id +
                                    " | Channel: " +
                                    voiceState.channel_id
                            );
            ...
            case "READY": {
                ...
                this.client.user = this.client.users.update(
                    new ExtendedUser(packet.d.user, this.client),
                    this.client
                );
                if (this.client.user.bot) {
                    ...
                    if (packet.d.user_guild_settings) {
                        packet.d.user_guild_settings.forEach(
                            (guildSettings) => {
                                this.client.userGuildSettings[
                ...
                packet.d.guilds.forEach((guild) => {
                    if (guild.unavailable) {
                        ...
                        this.client.unavailableGuilds.remove(
                ...
                if (packet.d.presences) {
                    packet.d.presences.forEach((presence) => {
                        if (this.client.relationships.get(presence.user.id)) {
                            ...
                            presence.id = presence.user.id;
                            this.client.relationships.update(
                                presence,
                                null,
                                true
                            );
            ...
            case "RELATIONSHIP_ADD": {
                ...
                if (relationship) {
                    ...
                    this.emit(
                        "relationshipAdd",
                        this.client.relationships.add(packet.d, this.client)
                    );
                }
                break;
            }
            case "RELATIONSHIP_REMOVE": {
                ...
                this.emit(
                    "relationshipRemove",
                    this.client.relationships.remove(packet.d)
                );
                break;
            }
            ...
            case "CHANNEL_PINS_UPDATE": {
                ...
                if (!channel) {
                    this.emit(
                        "debug",
                        `CHANNEL_PINS_UPDATE target channel ${packet.d.channel_id} not found`
                    );
                    break;
                }
            ...
            case "THREAD_CREATE": {
                ...
                if (!channel.guild) {
                    ...
                    if (!channel.guild) {
                        this.emit(
                            "debug",
                            `Received THREAD_CREATE for channel in missing guild ${packet.d.guild_id}`
                        );
                        break;
                    }
                }
            ...
            case "THREAD_UPDATE": {
                ...
                if (!channel) {
                    ...
                    this.emit(
                        ...
                        this.client.guilds
                            ...
                            .threads.add(thread, this.client),
                        null
                    );
                    this.client.threadGuildMap[packet.d.id] = packet.d.guild_id;
                    break;
                ...
                if (!(channel instanceof ThreadChannel)) {
                    this.emit(
                        "warn",
                        `Unexpected THREAD_UPDATE for channel ${packet.d.id} with type ${channel.type}`
                    );
                    break;
                }
                const oldChannel = {
            ...
            case "THREAD_DELETE": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in THREAD_DELETE`
                    );
                    break;
                }
            ...
            case "THREAD_LIST_SYNC": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in THREAD_LIST_SYNC`
                    );
                    break;
                }
                const deletedThreads = (
                    packet.d.channel_ids || guild.threads.map((c) => c.id)
                ) // REVIEW Is this a good name?
                    .filter((c) => !packet.d.threads.some((t) => t.id === c))
                    .map((id) => guild.threads.remove({ id }) || { id });
                const activeThreads = packet.d.threads.map((t) =>
                    guild.threads.update(t, this.client)
                );
            ...
            case "THREAD_MEMBER_UPDATE": {
                ...
                if (!channel) {
                    this.emit(
                        "debug",
                        `Missing channel ${packet.d.id} in THREAD_MEMBER_UPDATE`
                    );
                    break;
                }
                let oldMember = null;
                ...
                let member = channel.members.get(
                    (packet.d.id = packet.d.user_id)
                );
                if (member) {
                    oldMember = {
            ...
            case "THREAD_MEMBERS_UPDATE": {
                ...
                if (!channel) {
                    this.emit(
                        "debug",
                        `Missing channel ${packet.d.id} in THREAD_MEMBERS_UPDATE`
                    );
                    break;
                }
                channel.update(packet.d);
                ...
                if (packet.d.removed_member_ids) {
                    removedMembers = packet.d.removed_member_ids.map(
            ...
            case "STAGE_INSTANCE_CREATE": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in STAGE_INSTANCE_CREATE`
                    );
                    break;
                }
                /**
                ...
                this.emit(
                    "stageInstanceCreate",
                    guild.stageInstances.add(packet.d, this.client)
                );
                break;
            }
            case "STAGE_INSTANCE_UPDATE": {
                ...
                this.emit(
                    "stageInstanceUpdate",
                    guild.stageInstances.update(packet.d, this.client),
                    oldStageInstance
                );
                break;
            }
            ...
            case "GUILD_SCHEDULED_EVENT_CREATE": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in GUILD_SCHEDULED_EVENT_CREATE`
                    );
                    break;
                }
            ...
            case "GUILD_SCHEDULED_EVENT_DELETE": {
                ...
                if (!guild) {
                    this.emit(
                        "debug",
                        `Missing guild ${packet.d.guild_id} in GUILD_SCHEDULED_EVENT_DELETE`
                    );
                    break;
                }
            ...
            case "GUILD_SCHEDULED_EVENT_USER_ADD": {
                const guild =
                    this.client.guildEventMap[
                        packet.d.guild_scheduled_event_id
                    ];
                const user =
                    this.client.users.get(packet.d.user_id) || packet.d.user_id;
            ...
            case "INTERACTION_CREATE": {
                 ...
                 * @prop {PingInteraction | CommandInteraction | ComponentInteraction | AutocompleteInteraction | ModalSubmitInteraction} Interaction The Interaction that was created
                 */
                this.emit(
                    "interactionCreate",
                    Interaction.from(packet.d, this.client)
                );
                break;
            }
            default: {
    ...
    _onWSMessage(data) {
        try {
            ...
            if (this.client.options.compress) {
                if (
                    data.length >= 4 &&
                    data.readUInt32BE(data.length - 4) === 0xffff
                ) {
                    ...
                    if (this._zlibSync.err) {
                        this.emit(
                            "error",
                            new Error(
                                `zlib error ${this._zlibSync.err}: ${this._zlibSync.msg}`
                            )
                        );
                        return;
                    }
