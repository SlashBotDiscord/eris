...
class Command {
        ...
        this.reactionButtons = options.reactionButtons
            ? options.reactionButtons.map((button, index) => {
                  if (typeof button.response === "string") {
                      ...
                      button.responses = button.response.map(
                          (item, otherIndex) => {
                      ...
                      button.execute = () =>
                          button.responses[
                              Math.floor(
                                  Math.random() * button.responses.length
                              )
                          ]();
                      return button;
    ...
    get fullLabel() {
        return `${
            this.parentCommand ? this.parentCommand.fullLabel + " " : ""
        }${this.label}`;
    }
    ...
    async process(args, msg) {
        const shouldDelete =
            ...
            msg.channel.guild &&
            msg.channel
                .permissionsOf(msg._client.user.id)
                .has("manageMessages");
        ...
        if (args.length === 0) {
            ...
            if (this.argsRequired) {
                if (this.hooks.postCheck) {
                    const response = await this.hooks.postCheck(
                        msg,
                        args,
                        true
                    );
        ...
        if (
            ...
            ((subcommand = this.subcommands[label.toLowerCase()]) !==
                undefined &&
    ...
    registerSubcommand(label, generator, options = {}) {
        ...
        if (this.subcommands[label]) {
            throw new Error(
                "You have already registered a subcommand for " + label
            );
        }
