...
class CommandClient extends Client {
        ...
        this.once("shardPreReady", () => {
            ...
            if (Array.isArray(this.commandOptions.prefix)) {
                for (let i = 0; i < this.commandOptions.prefix.length; ++i) {
                    this.commandOptions.prefix[i] = this.commandOptions.prefix[
                        i
                    ].replace(/@mention/g, this.user.mention);
                }
            } else {
            ...
            for (const key of Object.keys(this.guildPrefixes)) {
                if (Array.isArray(this.guildPrefixes[key])) {
                    for (let i = 0; i < this.guildPrefixes[key].length; ++i) {
                        this.guildPrefixes[key][i] = this.guildPrefixes[key][
                            i
                        ].replace(/@mention/g, this.user.mention);
                    }
                } else {
            ...
            this.registerCommand(
                ...
                (msg, args) => {
                    ...
                    if (args.length > 0) {
                        let cur =
                            this.commands[
                                this.commandAliases[args[0]] || args[0]
                            ];
                        if (!cur) {
                        ...
                        for (let i = 1; i < args.length; ++i) {
                            cur =
                                cur.subcommands[
                                    cur.subcommandAliases[args[i]] || args[i]
                                ];
                            if (!cur) {
                                return "Command not found";
                            }
                        ...
                        if (subcommands.length > 0) {
                            ...
                            for (const subLabel of subcommands) {
                                if (
                                    ...
                                    cur.subcommands[subLabel].permissionCheck(
                                        msg
                                    )
                                ) {
    ...
    checkPrefix(msg) {
        ...
        if (typeof prefixes === "string") {
            return (
                msg.content.replace(/<@!/g, "<@").startsWith(prefixes) &&
                prefixes
            );
    ...
    async onMessageCreate(msg) {
        ...
        msg.command = false;
        if (
            (!this.commandOptions.ignoreSelf ||
            ...
            if (command !== undefined) {
                ...
                try {
                    ...
                    if (msg.command.errorMessage) {
                        try {
                            if (
                                typeof msg.command.errorMessage === "function"
                            ) {
                                const reply = await msg.command.errorMessage(
                                    msg,
                                    err
                                );
                                if (reply !== undefined) {
        ...
        if (activeMessage && activeMessage.command.reactionButtons) {
            ...
            switch (action.type) {
                case "cancel": {
                    this.unwatchMessage(
                        msg.id,
                        msg.channel.guild && msg.channel.id
                    );
                    try {
    ...
    registerCommand(label, generator, options = {}) {
        ...
        if (
            ...
            throw new Error(
                `You have already registered a command for ${label}`
            );
        }
