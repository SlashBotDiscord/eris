...
class RequestHandler {
    constructor(client, options) {
        ...
        this._client = client;
        this.userAgent = `DiscordBot (https://github.com/SlashBotDiscord/eris, 0.0.0)`;
        this.ratelimits = {};
        this.latencyRef = {
            ...
            const actualCall = (cb) => {
                ...
                try {
                    ...
                    if (body && body.reason) {
                        ...
                        if (this.options.decodeReasons) {
                            try {
                                if (
                                    unencodedReason.includes("%") &&
                                    !unencodedReason.includes(" ")
                                ) {
                                    unencodedReason =
                                        decodeURIComponent(unencodedReason);
                                }
                            } catch (err) {
                                this._client.emit("error", err);
                            }
                        }
                        headers["X-Audit-Log-Reason"] =
                            encodeURIComponent(unencodedReason);
                        if (
                ...
                req.once("abort", () => {
                    ...
                    reqError =
                        reqError ||
                        new Error(
                            `Request aborted by client on ${method} ${url}`
                        );
                    reqError.req = req;
                    reject(reqError);
                }).once("error", (err) => {
                ...
                req.once("response", (resp) => {
                    ...
                    resp.once("aborted", () => {
                        ...
                        reqError =
                            reqError ||
                            new Error(
                                `Request aborted by server on ${method} ${url}`
                            );
                        reqError.req = req;
                        reject(reqError);
                    });
                    ...
                    if (resp.headers["content-encoding"]) {
                        if (resp.headers["content-encoding"].includes("gzip")) {
                            ...
                            resp.headers["content-encoding"].includes("deflate")
                        ) {
                    ...
                    _respStream
                        ...
                        .once("end", () => {
                            ...
                            if (retryAfter >= 0) {
                                ...
                                let resetTime =
                                    +resp.headers["x-ratelimit-reset"] * 1000;
                                if (
                                ...
                                this.ratelimits[route].reset = Math.max(
                                    resetTime - this.latencyRef.latency,
                                    now
                                );
                            } else {
                ...
                req.setTimeout(this.options.requestTimeout, () => {
                    reqError = new Error(
                        `Request timed out (>${this.options.requestTimeout}ms) on ${method} ${url}`
                    );
                    req.abort();
                });
            ...
            if (this.globalBlock && auth) {
                this.readyQueue.push(() => {
                    if (!this.ratelimits[route]) {
                        this.ratelimits[route] = new SequentialBucket(
                            1,
                            this.latencyRef
                        );
                    }
    ...
    routefy(url, method) {
        ...
        if (method === "DELETE" && route.endsWith("/messages/:id")) {
            ...
            method === "GET" &&
            /\/guilds\/[0-9]+\/channels$/.test(route)
        ) {
