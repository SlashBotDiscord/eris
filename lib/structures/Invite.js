    ...
    constructor(data, client) {
        ...
        if (data.guild && client.guilds.has(data.guild.id)) {
            this.channel = client.guilds
                .get(data.guild.id)
                .channels.update(data.channel, client);
        } else {
        ...
        if (data.stage_instance !== undefined) {
            data.stage_instance.members = data.stage_instance.members.map(
                (m) => {
                    m.id = m.user.id;
                    return m;
                }
            ...
            this.stageInstance = {
                members: data.stage_instance.members.map((m) =>
                    this.guild.members.update(m, this.guild)
                ),
        ...
        this.targetApplicationID =
            data.target_application !== undefined
                ? data.target_application.id
                : null;
        this.targetType =
            data.target_type !== undefined ? data.target_type : null;
        this.targetUser =
            data.target_user !== undefined
                ? this._client.users.update(data.target_user, this._client)
                : null;
    }
