...
class GroupChannel extends PrivateChannel {
        ...
        data.recipients.forEach((recipient) => {
            this.recipients.add(
                client.options.restMode
                    ? new User(recipient, client)
                    : client.users.add(recipient, client)
            );
        });
        this.update(data);
    }
    ...
    get iconURL() {
        return this.icon
            ? this.client._formatImage(
                  Endpoints.CHANNEL_ICON(this.id, this.icon)
              )
            : null;
    }
    ...
    removeRecipient(userID) {
        return this.client.removeGroupRecipient.call(
            this.client,
            this.id,
            userID
        );
