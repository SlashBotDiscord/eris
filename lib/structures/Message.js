...
class Message extends Base {
    constructor(data, client) {
        ...
        if (data.author) {
            ...
            this._client.emit(
                "error",
                new Error(
                    "MESSAGE_CREATE but no message author:\n" +
        ...
        if (data.referenced_message) {
            const channel = this._client.getChannel(
                data.referenced_message.channel_id
            );
            if (channel) {
                this.referencedMessage = channel.messages.update(
                    data.referenced_message,
                    this._client
                );
            } else {
                this.referencedMessage = new Message(
                    data.referenced_message,
                    this._client
                );
            }
        ...
        if (data.interaction) {
            ...
            const interactionUser = this._client.users.update(
            ...
            if (data.interaction.member) {
                ...
                if (this.channel.guild) {
                    interactionMember = this.channel.guild.members.update(
                ...
                this.channel.guild &&
                this.channel.guild.members.has(data.interaction.user.id)
            ) {
                interactionMember = this.channel.guild.members.get(
        ...
        switch (this.type) {
            ...
            case MessageTypes.CALL: {
                if (data.call.ended_timestamp) {
                    if (
                        !this.channel.lastCall ||
                        this.channel.lastCall.endedTimestamp <
                            Date.parse(data.call.ended_timestamp)
                    ) {
                        data.call.id = this.id;
                        this.channel.lastCall = new Call(
                            data.call,
                            this.channel
                        );
                    }
            ...
            case MessageTypes.GUILD_MEMBER_JOIN: {
                data.content = SystemJoinMessages[
                    ~~(this.createdAt % SystemJoinMessages.length)
            ...
            case MessageTypes.USER_PREMIUM_GUILD_SUBSCRIPTION_TIER_3: {
                data.content = `${
                    ...
                    this.channel.guild ? this.channel.guild.name : data.guild_id
                } has achieved **Level ${this.type - 8}!**`;
                break;
            }
            ...
            default: {
                this._client.emit(
                    "warn",
                    `Unhandled MESSAGE_CREATE type: ${JSON.stringify(
                        data,
                        null,
                        2
                    )}`
    ...
    update(data, client) {
        ...
        if (data.content !== undefined) {
            ...
            this.mentions = data.mentions.map((mention) => {
                ...
                if (mention.member && this.channel.guild) {
                    ...
                    this.channel.guild.members.update(
                        mention.member,
                        this.channel.guild
                    );
                }
        ...
        if (data.sticker_items !== undefined) {
            this.stickerItems = data.sticker_items.map((sticker) => {
                if (sticker.user) {
                    sticker.user = this._client.users.update(
                        sticker.user,
                        client
                    );
                }
    ...
    get channelMentions() {
        ...
        return (this._channelMentions = (
            (this.content && this.content.match(/<#[0-9]+>/g)) ||
            []
        ).map((mention) => mention.substring(2, mention.length - 1)));
    }

    get jumpLink() {
        return `${Endpoints.CLIENT_URL}${Endpoints.MESSAGE_LINK(
            this.channel.type === 1 ? "@me" : this.channel.guild.id,
            this.channel.id,
            this.id
        )}`;
    }
    ...
    addReaction(reaction, userID) {
        ...
        return this._client.addMessageReaction.call(
            this._client,
            this.channel.id,
            this.id,
            reaction,
    ...
    createThreadWithMessage(options) {
        return this._client.createThreadWithMessage.call(
            this._client,
            this.channel.id,
            this.id,
            options
    ...
    crosspost() {
        ...
        return this._client.crosspostMessage.call(
            this._client,
            this.channel.id,
            this.id
        );
    ...
    delete(reason) {
        ...
        return this._client.deleteMessage.call(
            this._client,
            this.channel.id,
            this.id,
            reason
    ...
    edit(content) {
        if (this.flags & MessageFlags.EPHEMERAL) {
            throw new Error(
                "Ephemeral messages cannot be edited via this method"
            );
        }
        return this._client.editMessage.call(
            this._client,
            this.channel.id,
            this.id,
            content
    ...
    getReaction(reaction, options, before, after) {
        ...
        return this._client.getMessageReaction.call(
            this._client,
            this.channel.id,
            this.id,
            reaction,
    ...
    pin() {
        ...
        return this._client.pinMessage.call(
            this._client,
            this.channel.id,
            this.id
        );
    ...
    removeReaction(reaction, userID) {
        ...
        return this._client.removeMessageReaction.call(
            this._client,
            this.channel.id,
            this.id,
            reaction,
    ...
    removeReactionEmoji(reaction) {
        ...
        return this._client.removeMessageReactionEmoji.call(
            this._client,
            this.channel.id,
            this.id,
            reaction
    ...
    removeReactions() {
        ...
        return this._client.removeMessageReactions.call(
            this._client,
            this.channel.id,
            this.id
        );
    ...
    unpin() {
        ...
        return this._client.unpinMessage.call(
            this._client,
            this.channel.id,
            this.id
        );
