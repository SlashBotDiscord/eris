...
class Client extends EventEmitter {
    ...
    constructor(token, options) {
        ...
        this.options = Object.assign(
            {
                ...
                restMode: false,
                ws: {},
                reconnectDelay: (lastDelay, attempts) =>
                    Math.pow(attempts + 1, 0.7) * 20000,
            },
            options
        );
        this.options.allowedMentions = this._formatAllowedMentions(
            this.options.allowedMentions
        );
        if (
            this.options.lastShardID === undefined &&
            this.options.maxShards !== "auto"
        ) {
            this.options.lastShardID = this.options.maxShards - 1;
        }
        if (
            !Constants.ImageFormats.includes(
                this.options.defaultImageFormat.toLowerCase()
            )
        ) {
            throw new TypeError(
                `Invalid default image format: ${this.options.defaultImageFormat}`
            );
        }
        ...
        if (
            ...
            throw new TypeError(
                `Invalid default image size: ${defaultImageSize}`
            );
        }
        ...
        if (this.options.hasOwnProperty("intents")) {
            ...
            if (
                this.options.getAllUsers &&
                !(this.options.intents & Constants.Intents.guildMembers)
            ) {
                throw new Error(
                    "Cannot request all members without guildMembers intent"
                );
            }
        }
    ...
    acceptInvite(inviteID) {
        return this.requestHandler
            .request("POST", Endpoints.INVITE(inviteID), true)
    ...
    addGroupRecipient(groupID, userID) {
        return this.requestHandler.request(
            "PUT",
            Endpoints.CHANNEL_RECIPIENT(groupID, userID),
            true
        );
    }
    ...
    addGuildDiscoverySubcategory(guildID, categoryID, reason) {
        return this.requestHandler.request(
            "POST",
            Endpoints.GUILD_DISCOVERY_CATEGORY(guildID, categoryID),
            true,
            { reason }
        );
    }
    ...
    addRelationship(userID, block) {
        return this.requestHandler.request(
            "PUT",
            Endpoints.USER_RELATIONSHIP("@me", userID),
            true,
            {
    ...
    addSelfPremiumSubscription(token, plan) {
        return this.requestHandler.request(
            "PUT",
            Endpoints.USER_BILLING_PREMIUM_SUBSCRIPTION("@me"),
            true,
            {
                token: token,
    ...
    banGuildMember(guildID, userID, deleteMessageDays, reason) {
        if (
            !isNaN(deleteMessageDays) &&
            (deleteMessageDays < 0 || deleteMessageDays > 7)
        ) {
            return Promise.reject(
                new Error(
                    `Invalid deleteMessageDays value (${deleteMessageDays}), should be a number between 0-7 inclusive`
                )
            );
        }
        return this.requestHandler.request(
    ...
    bulkEditCommandPermissions(guildID, permissions) {
        if (!guildID) {
            throw new Error(
                "You must provide an id of the guild whose permissions you want to edit."
            );
        }

        return this.requestHandler.request(
            "PUT",
            Endpoints.GUILD_COMMAND_PERMISSIONS(this.application.id, guildID),
            true,
            permissions
        );
    }
    ...
    bulkEditCommands(commands) {
            ...
            if (command.name !== undefined) {
                if (command.type === 1 || command.type === undefined) {
                    ...
                    if (!command.name.match(/^[\w-]{1,32}$/)) {
                        throw new Error(
                            'Slash Command names must match the regular expression "^[\\w-]{1,32}$"'
                        );
                    }
                }
            }
        ...
        return this.requestHandler.request(
            "PUT",
            Endpoints.COMMANDS(this.application.id),
            true,
            commands
        );
    }
    ...
    bulkEditGuildCommands(guildID, commands) {
        ...
        return this.requestHandler.request(
            "PUT",
            Endpoints.GUILD_COMMANDS(this.application.id, guildID),
            true,
            commands
        );
    }
    ...
    async connect() {
        try {
            ...
            if (
                !data.url ||
                (this.options.maxShards === "auto" && !data.shards)
            ) {
    ...
    createChannel(guildID, name, type, options = {}) {
        return this.requestHandler
            .request("POST", Endpoints.GUILD_CHANNELS(guildID), true, {
                name: name,
                type: type,
    ...
    createChannelInvite(channelID, options = {}, reason) {
        return this.requestHandler
            .request("POST", Endpoints.CHANNEL_INVITES(channelID), true, {
    ...
    createGroupChannel(userIDs) {
        return this.requestHandler
            .request("POST", Endpoints.USER_CHANNELS("@me"), true, {
                recipients: userIDs,
                type: 3,
            })
    ...
    createGuild(name, options) {
        if (this.guilds.size > 9) {
            throw new Error(
                "This method can't be used when in 10 or more guilds."
            );
        }

        return this.requestHandler
            .request("POST", Endpoints.GUILDS, true, {
                name: name,
                icon: options.icon,
    ...
    createGuildEvent(guildID, event) {
        return this.requestHandler
            .request("POST", Endpoints.GUILD_SCHEDULED_EVENTS(guildID), true, {
                name: event.name,
    ...
    createGuildFromTemplate(code, name, icon) {
        return this.requestHandler
            .request("POST", Endpoints.GUILD_TEMPLATE(code), true, {
                name,
                icon,
            })
            .then((guild) => new Guild(guild, this));
    ...
    createGuildSticker(guildID, options, reason) {
        return this.requestHandler.request(
            "POST",
            Endpoints.GUILD_STICKERS(guildID),
            true,
            {
    ...
    createGuildTemplate(guildID, name, description) {
        return this.requestHandler
            .request("POST", Endpoints.GUILD_TEMPLATES(guildID), true, {
                name,
                description,
            })
    ...
    createInteractionResponse(interactionID, interactionToken, options, file) {
        ...
        return this.requestHandler.request(
            "POST",
            Endpoints.INTERACTION_RESPOND(interactionID, interactionToken),
            true,
            options,
            file,
    ...
    createMessage(channelID, content, file) {
        if (content !== undefined) {
            ...
            content.allowed_mentions = this._formatAllowedMentions(
    ...
    createRole(guildID, options, reason) {
        if (options.permissions !== undefined) {
            options.permissions =
                options.permissions instanceof Permission
                    ? String(options.permissions.allow)
                    : String(options.permissions);
        }
        return this.requestHandler
            .request("POST", Endpoints.GUILD_ROLES(guildID), true, {
                name: options.name,
    ...
    createStageInstance(channelID, options) {
        return this.requestHandler
            .request("POST", Endpoints.STAGE_INSTANCES, true, {
    ...
    createThreadWithMessage(channelID, messageID, options) {
        return this.requestHandler
            .request(
                "POST",
                Endpoints.THREAD_WITH_MESSAGE(channelID, messageID),
                true,
                {
                    name: options.name,
    ...
    createThreadWithoutMessage(channelID, options) {
        return this.requestHandler
            .request(
                "POST",
                Endpoints.THREAD_WITHOUT_MESSAGE(channelID),
                true,
                {
                    auto_archive_duration: options.autoArchiveDuration,
    ...
    crosspostMessage(channelID, messageID) {
        return this.requestHandler
            .request(
                "POST",
                Endpoints.CHANNEL_CROSSPOST(channelID, messageID),
                true
            )
            .then((message) => new Message(message, this));
    }
    ...
    deleteCommand(commandID) {
        ...
        return this.requestHandler.request(
            "DELETE",
            Endpoints.COMMAND(this.application.id, commandID),
            true
        );
    }
    ...
    deleteGuildCommand(guildID, commandID) {
        if (!guildID) {
            throw new Error(
                "You must provide an id of the guild which the command is in."
            );
        }
        if (!commandID) {
            throw new Error("You must provide an id of the command to delete.");
        ...
        return this.requestHandler.request(
            "DELETE",
            Endpoints.GUILD_COMMAND(this.application.id, guildID, commandID),
            true
        );
    }
    ...
    deleteGuildEvent(guildID, eventID) {
        return this.requestHandler.request(
            "DELETE",
            Endpoints.GUILD_SCHEDULED_EVENT(guildID, eventID),
            true
        );
    }
    /**
    ...
    deleteGuildSticker(guildID, stickerID, reason) {
        return this.requestHandler.request(
            "DELETE",
            Endpoints.GUILD_STICKER(guildID, stickerID),
            true,
            {
                reason,
            }
    ...
    deleteGuildTemplate(guildID, code) {
        return this.requestHandler
            .request(
                "DELETE",
                Endpoints.GUILD_TEMPLATE_GUILD(guildID, code),
                true
            )
            .then((template) => new GuildTemplate(template, this));
    }
    ...
    deleteMessages(channelID, messageIDs, reason) {
        ...
        const invalidMessage = messageIDs.find(
            (messageID) => messageID < oldestAllowedSnowflake
        );
        if (invalidMessage) {
            return Promise.reject(
                new Error(`Message ${invalidMessage} is more than 2 weeks old.`)
            );
        }
    ...
    deleteSelfConnection(platform, id) {
        return this.requestHandler.request(
            "DELETE",
            Endpoints.USER_CONNECTION_PLATFORM("@me", platform, id),
            true
        );
    }
    ...
    deleteWebhookMessage(webhookID, token, messageID) {
        return this.requestHandler.request(
            "DELETE",
            Endpoints.WEBHOOK_MESSAGE(webhookID, token, messageID),
            false
        );
    }
    ...
    disableSelfMFATOTP(code) {
        return this.requestHandler
            .request("POST", Endpoints.USER_MFA_TOTP_DISABLE("@me"), true, {
                code,
            })
            .then((data) => {
                if (data.token) {
     ...
     * @arg {String?} [options.parentID] The ID of the parent channel category for this channel (guild text channels only)
     ...
     * @arg {String} [reason] The reason to be displayed in audit logs
     * @returns {Promise<CategoryChannel | GroupChannel | TextChannel | NewsChannel | NewsThreadChannel | PrivateThreadChannel | PublicThreadChannel>}
     */
    editChannel(channelID, options, reason) {
        return this.requestHandler
            .request("PATCH", Endpoints.CHANNEL(channelID), true, {
    ...
    editChannelPosition(channelID, position, options = {}) {
        let channels = this.guilds.get(
            this.channelGuildMap[channelID]
        ).channels;
        ...
        return this.requestHandler.request(
            "PATCH",
            Endpoints.GUILD_CHANNELS(this.channelGuildMap[channelID]),
            true,
    ...
    editCommandPermissions(guildID, commandID, permissions) {
        ...
        if (!commandID) {
            throw new Error(
                "You must provide an id of the command whose permissions you want to edit."
            );
        }
        return this.requestHandler.request(
            "PUT",
            Endpoints.COMMAND_PERMISSIONS(
                this.application.id,
                guildID,
                commandID
            ),
    ...
    editGuild(guildID, options, reason) {
        return this.requestHandler
            .request("PATCH", Endpoints.GUILD(guildID), true, {
                name: options.name,
                icon: options.icon,
                verification_level: options.verificationLevel,
                default_message_notifications: options.defaultNotifications,
    ...
    editGuildMember(guildID, memberID, options, reason) {
        return this.requestHandler
            .request("PATCH", Endpoints.GUILD_MEMBER(guildID, memberID), true, {
                roles:
                    options.roles &&
                    options.roles.filter(
                        (roleID, index) =>
                            options.roles.indexOf(roleID) === index
                    ),
                nick: options.nick,
                mute: options.mute,
                deaf: options.deaf,
            ...
            .then(
                (member) => new Member(member, this.guilds.get(guildID), this)
            );
    }
    ...
    editGuildVoiceState(guildID, options, userID = "@me") {
        return this.requestHandler.request(
            "PATCH",
            Endpoints.GUILD_VOICE_STATE(guildID, userID),
            true,
            {
                channel_id: options.channelID,
    ...
    editSelf(options) {
        return this.requestHandler
            .request("PATCH", Endpoints.USER("@me"), true, options)
    ...
    editWebhookMessage(webhookID, token, messageID, options) {
        if (options.allowedMentions) {
            options.allowed_mentions = this._formatAllowedMentions(
    ...
    enableSelfMFATOTP(secret, code) {
        return this.requestHandler
            .request("POST", Endpoints.USER_MFA_TOTP_ENABLE("@me"), true, {
                secret,
                code,
            })
            .then((data) => {
    ...
    executeSlackWebhook(webhookID, token, options) {
        ...
        return this.requestHandler.request(
            "POST",
            Endpoints.WEBHOOK_TOKEN_SLACK(webhookID, token) +
                (qs ? "?" + qs : ""),
            auth,
            options
        );
    ...
    executeWebhook(webhookID, token, options) {
        ...
        return this.requestHandler
            ...
            .then((response) =>
                options.wait ? new Message(response, this) : undefined
            );
    }
    ...
    followChannel(channelID, webhookChannelID) {
        return this.requestHandler.request(
            "POST",
            Endpoints.CHANNEL_FOLLOW(channelID),
            true,
    ...
    getActiveGuildThreads(guildID) {
        return this.requestHandler
            .request("GET", Endpoints.THREADS_GUILD_ACTIVE(guildID), true)
            .then((response) => {
                return {
                    members: response.members.map(
                        (member) => new ThreadMember(member, this)
                    ),
                    threads: response.threads.map((thread) =>
                        Channel.from(thread, this)
                    ),
                };
            });
    }
    ...
    getArchivedThreads(channelID, type, options = {}) {
        return this.requestHandler
            .request(
                "GET",
                Endpoints.THREADS_ARCHIVED(channelID, type),
                true,
                options
            )
            .then((response) => {
    ...
    getChannel(channelID) {
        ...
        if (
            this.channelGuildMap[channelID] &&
            this.guilds.get(this.channelGuildMap[channelID])
        ) {
            return this.guilds
                .get(this.channelGuildMap[channelID])
                .channels.get(channelID);
        }
        if (
            this.threadGuildMap[channelID] &&
            this.guilds.get(this.threadGuildMap[channelID])
        ) {
            return this.guilds
                .get(this.threadGuildMap[channelID])
                .threads.get(channelID);
        }
        return (
            this.privateChannels.get(channelID) ||
            this.groupChannels.get(channelID)
        );
    }
    ...
    getChannelInvites(channelID) {
        return this.requestHandler
            .request("GET", Endpoints.CHANNEL_INVITES(channelID), true)
            .then((invites) =>
                invites.map((invite) => new Invite(invite, this))
            );
    }
    ...
    getCommandPermissions(guildID, commandID) {
        if (!guildID) {
            throw new Error(
                "You must provide an id of the guild whose permissions you want to get."
            );
        }
        if (!commandID) {
            throw new Error(
                "You must provide an id of the command whose permissions you want to get."
            );
        }
        return this.requestHandler.request(
            "GET",
    ...
    getDMChannel(userID) {
        if (this.privateChannelMap[userID]) {
            return Promise.resolve(
                this.privateChannels.get(this.privateChannelMap[userID])
            );
        }
        return this.requestHandler
            .request("POST", Endpoints.USER_CHANNELS("@me"), true, {
    ...
    getEmojiGuild(emojiID) {
        return this.requestHandler
            .request("GET", Endpoints.CUSTOM_EMOJI_GUILD(emojiID), true)
    ...
    getGuildAuditLog(guildID, options = {}, before, actionType, userID) {
        ...
        return this.requestHandler
            .request("GET", Endpoints.GUILD_AUDIT_LOGS(guildID), true, options)
            .then((data) => {
                const guild = this.guilds.get(guildID);
                return {
                    entries: data.audit_log_entries.map(
                        (entry) => new GuildAuditLogEntry(entry, guild)
                    ),
                    ...
                    threads: data.threads.map((thread) =>
                        guild.threads.update(thread, this)
                    ),
    ...
    getGuildBan(guildID, userID) {
        return this.requestHandler
            .request("GET", Endpoints.GUILD_BAN(guildID, userID), true)
            .then((ban) => {
    ...
    getGuildBans(guildID) {
        return this.requestHandler
            .request("GET", Endpoints.GUILD_BANS(guildID), true)
            .then((bans) => {
                bans.forEach((ban) => {
                    ban.user = new User(ban.user, this);
                });
    ...
    getGuildIntegrations(guildID) {
        const guild = this.guilds.get(guildID);
        return this.requestHandler
            .request("GET", Endpoints.GUILD_INTEGRATIONS(guildID), true)
            .then((integrations) =>
                integrations.map(
                    (integration) => new GuildIntegration(integration, guild)
                )
            );
    }
    ...
    getGuildInvites(guildID) {
        return this.requestHandler
            .request("GET", Endpoints.GUILD_INVITES(guildID), true)
            .then((invites) =>
                invites.map((invite) => new Invite(invite, this))
            );
    }
    ...
    getGuildPreview(guildID) {
        return this.requestHandler
            .request("GET", Endpoints.GUILD_PREVIEW(guildID), true)
    ...
    getGuildTemplate(code) {
        return this.requestHandler
            .request("GET", Endpoints.GUILD_TEMPLATE(code), true)
            .then((template) => new GuildTemplate(template, this));
    }
    ...
    getGuildTemplates(guildID) {
        return this.requestHandler
            .request("GET", Endpoints.GUILD_TEMPLATES(guildID), true)
            .then((templates) =>
                templates.map((t) => new GuildTemplate(t, this))
            );
    }
    ...
    getGuildWidgetSettings(guildID) {
        return this.requestHandler.request(
            "GET",
            Endpoints.GUILD_WIDGET_SETTINGS(guildID),
            true
        );
    }
    ...
    getInvite(inviteID, withCounts) {
        return this.requestHandler
            .request("GET", Endpoints.INVITE(inviteID), true, {
    ...
    getJoinedPrivateArchivedThreads(channelID, options = {}) {
        return this.requestHandler
            .request(
                "GET",
                Endpoints.THREADS_ARCHIVED_JOINED(channelID),
                true,
                options
            )
            .then((response) => {
            ...
            const get = async (_before, _after) => {
                ...
                logs = _after
                    ? messages
                          ...
                          .concat(logs)
                    : logs.concat(
                          messages.map((message) => new Message(message, this))
                      );
                ...
                return get(
                    (_before || !_after) && messages[messages.length - 1].id,
                    _after && messages[0].id
                );
            };
    ...
    getOAuthApplication(appID) {
        return this.requestHandler.request(
            "GET",
            Endpoints.OAUTH2_APPLICATION(appID || "@me"),
            true
        );
    }
    ...
    getPins(channelID) {
        return this.requestHandler
            .request("GET", Endpoints.CHANNEL_PINS(channelID), true)
            .then((messages) =>
                messages.map((message) => new Message(message, this))
            );
    }
    ...
    getPruneCount(guildID, options = {}) {
        return this.requestHandler
            .request("GET", Endpoints.GUILD_PRUNE(guildID), true, {
                days: options.days,
    ...
    getRESTChannel(channelID) {
        ...
        return this.requestHandler
            .request("GET", Endpoints.CHANNEL(channelID), true)
            .then((channel) => Channel.from(channel, this));
    }
    ...
    getRESTGuild(guildID, withCounts = false) {
        ...
        return this.requestHandler
            .request("GET", Endpoints.GUILD(guildID), true, {
                with_counts: withCounts,
            })
            .then((guild) => new Guild(guild, this));
    }
    ...
    getRESTGuildChannels(guildID) {
        ...
        return this.requestHandler
            .request("GET", Endpoints.GUILD_CHANNELS(guildID), true)
            .then((channels) =>
                channels.map((channel) => Channel.from(channel, this))
            );
    }
    ...
    getRESTGuildMember(guildID, memberID) {
        ...
        return this.requestHandler
            .request("GET", Endpoints.GUILD_MEMBER(guildID, memberID), true)
            .then(
                (member) => new Member(member, this.guilds.get(guildID), this)
            );
    }
    ...
    getRESTGuildMembers(guildID, options = {}) {
        ...
        return this.requestHandler
            .request("GET", Endpoints.GUILD_MEMBERS(guildID), true, options)
            .then((members) =>
                members.map(
                    (member) =>
    ...
    getRESTGuildRoles(guildID) {
        ...
        return this.requestHandler
            .request("GET", Endpoints.GUILD_ROLES(guildID), true)
    ...
    getRESTGuilds(options = {}) {
        ...
        return this.requestHandler
            .request("GET", Endpoints.USER_GUILDS("@me"), true, options)
    ...
    getRESTUser(userID) {
        ...
        return this.requestHandler
            .request("GET", Endpoints.USER(userID), true)
    ...
    getSelf() {
        return this.requestHandler
            .request("GET", Endpoints.USER("@me"), true)
            .then((data) => new ExtendedUser(data, this));
    }
    ...
    getSelfPayments() {
        return this.requestHandler.request(
            "GET",
            Endpoints.USER_BILLING_PAYMENTS("@me"),
            true
        );
    }
    ...
    getStageInstance(channelID) {
        return this.requestHandler
            .request("GET", Endpoints.STAGE_INSTANCE(channelID), true)
            .then((instance) => new StageInstance(instance, this));
    }
    ...
    getThreadMembers(channelID) {
        return this.requestHandler
            .request("GET", Endpoints.THREAD_MEMBERS(channelID), true)
            .then((members) =>
                members.map((member) => new ThreadMember(member, this))
            );
    }
    ...
    getVoiceRegions(guildID) {
        return guildID
            ...
            : this.requestHandler.request("GET", Endpoints.VOICE_REGIONS, true);
    }
    ...
    joinThread(channelID, userID = "@me") {
        return this.requestHandler.request(
            "PUT",
            Endpoints.THREAD_MEMBER(channelID, userID),
            true
        );
    }
    ...
    leaveGuild(guildID) {
        return this.requestHandler.request(
            "DELETE",
            Endpoints.USER_GUILD("@me", guildID),
            true
        );
    }
    ...
    listGuildEvents(guildID, withUserCount) {
        return this.requestHandler
            .request("GET", Endpoints.GUILD_SCHEDULED_EVENTS(guildID), true, {
                with_user_count: withUserCount,
            })
            .then((events) =>
                events.map((event) => new GuildEvent(event, this))
            );
    }
    ...
    listGuildEventUsers(guildID, eventID) {
        return this.requestHandler.request(
            "GET",
            Endpoints.GUILD_SCHEDULED_EVENT_USERS(guildID, eventID)
        );
    }
    ...
    pruneMembers(guildID, options = {}) {
        return this.requestHandler
            .request("POST", Endpoints.GUILD_PRUNE(guildID), true, {
                days: options.days,
    ...
    async purgeChannel(channelID, options) {
        ...
        const checkToDelete = async () => {
            const messageIDs =
                (done && toDelete) ||
                (toDelete.length >= 100 && toDelete.splice(0, 100));
            if (messageIDs) {
    ...
    searchChannelMessages(channelID, query) {
        return this.requestHandler
            .request(
                "GET",
                Endpoints.CHANNEL_MESSAGES_SEARCH(channelID),
                true,
                {
            ...
            .then((results) => ({
                ...
                results:
                    ...
                    results.messages.map((result) =>
                        result.map((message) => new Message(message, this))
                    ),
            }));
    }
    ...
    searchGuildMembers(guildID, query, limit) {
        return this.requestHandler
            .request("GET", Endpoints.GUILD_MEMBERS_SEARCH(guildID), true, {
                query,
                limit,
            })
            .then((members) => {
    ...
    searchGuildMessages(guildID, query) {
        return this.requestHandler
            .request("GET", Endpoints.GUILD_MESSAGES_SEARCH(guildID), true, {
                sort_by: query.sortBy,
                sort_order: query.sortOrder,
                content: query.content,
                author_id: query.authorID,
    ...
    syncGuildIntegration(guildID, integrationID) {
        return this.requestHandler.request(
            "POST",
            Endpoints.GUILD_INTEGRATION_SYNC(guildID, integrationID),
            true
        );
    }
    ...
    syncGuildTemplate(guildID, code) {
        return this.requestHandler
            .request("PUT", Endpoints.GUILD_TEMPLATE_GUILD(guildID, code), true)
            .then((template) => new GuildTemplate(template, this));
    }
    ...
    validateDiscoverySearchTerm(term) {
        return this.requestHandler.request(
            "GET",
            Endpoints.DISCOVERY_VALIDATION + `?term=${encodeURI(term)}`,
            true
        );
    }
